...
class AverageMovieRating:
    ...
    def read(self):
        lines = self.load()
        ratings = lines.map(lambda x: (x.split()[2], x.split()[1]))
        total_rating_by_movie = ratings.mapValues(lambda x: (x, 1)).reduceByKey(lambda x, y: (x[0] + y[0], x[1] + y[1]))
